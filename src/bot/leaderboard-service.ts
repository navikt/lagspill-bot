...
import { computeCurrentMonthLeaders, computeMonthlyWinners, detectMonthTransitionWinner } from '../utils/leaderboard'
...
export async function buildLeaderboardBlocks(
    ...
    const newMonthWinner = currentGameDate
        ? detectMonthTransitionWinner(finishedGames, currentGameDate, year, now)
        : null

    return leaderboardBlocks(gameCategory.name, currentMonthLeaders, monthWinners, year, now, newMonthWinner)
}
